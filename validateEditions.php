...
if (isset($_POST['edit'])) {
    ...
    if (isset($_POST['user-id'])) {
        ...
        for ($i = 0; $i < count($userIds); $i++) {
            ...
            $update = "UPDATE usuarios SET nombre='$name', email='$email', telefono='$phone' WHERE id=$id";
            $result = mysqli_query($connec, $update);
        }

        if ($result) {
            echo "<h3>Actualización Exitosa (Recarge el index.php para visualizar los cambios) </h3>";
        } else {
